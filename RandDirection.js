...
onmessage = (evt) => {
    ...
    let X = [...new Array(count)].map(() =>Math.round(Math.random() *(canvas.width-20)));
    let dX =  [...new Array(count)].map(() => (Math.round(Math.random()))?1:-1);
    let Y =  [...new Array(count)].map(() =>Math.round(Math.random() *(canvas.height-20)));
    let dY =[...new Array(count)].map(() => (Math.round(Math.random()))?1:-1);
